...
function processIcon(currentSize, icon, buffer, mimeType, publicPath, shouldFingerprint) {
  ...
  var fileName = shouldFingerprint ? 'icon_' + dimensions + '.' + (0, _fingerprint2.default)(buffer) + '.' + _mime2.default.getExtension(mimeType) : 'icon_' + dimensions + '.' + _mime2.default.getExtension(mimeType);
...
function process(sizes, icon, cachedIconsCopy, icons, assets, fingerprint, publicPath, callback) {
  ...
  var size = sizes.pop();
  if (size > 0) {
    var mimeType = _mime2.default.getType(icon.src);
